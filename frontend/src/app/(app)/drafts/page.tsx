...
function ListaDrafts({ companyId }: { companyId: string }) {
  ...
  const duplicar = useDuplicarEncarte(companyId);
  const { mostrar } = useToast();
  const [confirmando, setConfirmando] = useState<EncarteDraftResumo | null>(
    null
  );
  const [busca, setBusca] = useState("");
  const [formatoFiltro, setFormatoFiltro] = useState<number | "todos">("todos");
  ...
  const formatosDisponiveis = Array.from(
    new Set(data.map((d) => d.selectedFormat))
  ).sort((a, b) => a - b);

  const filtrados = data.filter((draft) => {
    const bateBusca = draft.name
      .toLowerCase()
      .includes(busca.trim().toLowerCase());
    const bateFormato =
      formatoFiltro === "todos" || draft.selectedFormat === formatoFiltro;
    return bateBusca && bateFormato;
  });
  ...
  async function duplicarDraft(draft: EncarteDraftResumo) {
    try {
      await duplicar.mutateAsync(draft.id);
      mostrar("sucesso", `"${draft.name}" duplicado.`);
    } catch (err) {
      mostrar(
        "erro",
    ...
    <>
      <div className="mb-5 flex flex-wrap items-center gap-3">
        <input
          ...
          className="min-h-10 flex-1 rounded-xl border border-[rgb(var(--line))] bg-[rgb(var(--surface))] px-3.5 py-2 text-sm outline-none placeholder:text-neutral-400 focus:border-[rgb(var(--brand)/0.5)]"
        />
        {formatosDisponiveis.length > 1 ? (
          <select
            ...
            className="min-h-10 rounded-xl border border-[rgb(var(--line))] bg-[rgb(var(--surface))] px-3 py-2 text-sm outline-none focus:border-[rgb(var(--brand)/0.5)]"
          >
      ...
      {filtrados.length === 0 ? (
        <SecaoVazia>Nenhum encarte encontrado com esse filtro.</SecaoVazia>
      ) : (
        <ul
                ...
                >
                  ...
                  <div className="p-4">
                    ...
                    <div className="mt-4 flex gap-2 border-t border-[rgb(var(--line))] pt-4">
                      ...
                      <Button
                        variante="fantasma"
                        className="h-10 min-h-10 w-10 shrink-0 px-0 text-[rgb(var(--brand))] hover:bg-[rgb(var(--brand)/0.1)]"
                        disabled={duplicar.isPending}
                        onClick={() => void duplicarDraft(draft)}
